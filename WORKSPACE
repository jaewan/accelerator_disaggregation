...
python_configure(
    ...
    python_version = "3",  # Use "3" for Python 3.x (not "3.8")
)
...
# Libtorch repository rule
load("//:libtorch_repository.bzl", "libtorch_repository")

libtorch_repository(
    name = "libtorch",
    pytorch_version = "2.5.1",  # Default PyTorch version
    cuda_tag = "cu121",        # Default CUDA tag (use "cpu" for CPU-only)
    sha256 = "",               # Add SHA256 for verification (optional)
)
